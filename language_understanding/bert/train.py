...
class Trainer(object):
    ...
    def train(self, get_loss,set_rate,p,a, model_file=None, pretrain_file=None, data_parallel=True):
        """ Train Loop """
        ...
        # self.cfg.total_steps=  self.cfg.n_epochs*math.ceil(self.data_iter.dataset.tensors[0].shape[0]/int(32) )
        ...
        for e in range(self.cfg.n_epochs):
            ...
            for i, batch in enumerate(iter_bar):
                ...
                if p == 0:
                    loss, _  = get_loss(model, batch, global_step)
                    loss = loss.mean() # mean() for Data Parallelism
                else:

                    set_rate(model,p)
                    loss1 = get_loss(model, batch, global_step).mean() # mean() for Data Parallelism
                    set_rate(model,0)
                    loss2 = get_loss(model, batch, global_step).mean() # mean() for Data Parallelism
                    # gap = torch.sigmoid(loss1-loss2)-0.5
                    gap = a*(loss1-loss2)/2
                    loss = loss2 + torch.tanh(gap)
